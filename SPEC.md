...
- **타임프레임**:
  ...
  - 전략 신호: 1h (원본)
  - 체결/SL/TP 검증: 1m (원본)
- **기간**: 사용자 지정 (기본: 2026-01-01 ~ 2026-03-25, 마지막 설정값을 sessionStorage에 기억)
- **데이터 동기화**: 1h, 1m 두 타임프레임 모두 동기화. 실제 DB 캔들 수가 기대값의 90% 미만이면 해당 구간을 강제 재수집 (gap detection)
...
4. **캔들 반전 패턴 (Confirmation)**: 신호 캔들이 **망치형(Hammer, wick_ratio ≥ 1.0)** 또는 직전 음봉의 몸통을 50% 이상 덮는 **상승 장악형 양봉** — 또는 신호 다음 봉(진입 봉)이 양봉인 경우도 허용.
