...
class TestBenchmarkLeaderboard:
    def test_leaderboard_page_loads(self, page: Page):
        """리더보드 페이지가 정상 로드된다."""
        page.goto(LEADERBOARD_URL, wait_until="load")
        page.wait_for_timeout(500)
        _wait_spinner(page)
        expect(page.locator("body")).to_be_visible()

    def test_leaderboard_renders_content(self, page: Page):
        """리더보드에 모델 목록, 빈 상태 메시지, 또는 로딩 표시가 있다."""
        page.goto(LEADERBOARD_URL, wait_until="load")
        page.wait_for_timeout(1000)
        body_text = page.locator("body").inner_text()
        has_content = (
            "리더보드" in body_text or
            "LOADING" in body_text or
            "모델" in body_text or
            "PROFIT LAB" in body_text
        )
        assert has_content, "리더보드 페이지 컨텐츠가 없습니다"
    ...
    def test_leaderboard_has_rankings_when_models_exist(self, page: Page):
        """모델이 있을 때 리더보드 헤딩 또는 순위 관련 텍스트가 표시된다."""
        page.goto(LEADERBOARD_URL, wait_until="load")
        page.wait_for_timeout(1500)
        body_text = page.locator("body").inner_text()
        # 모델이 없으면 skip
        if "LOADING" in body_text and "모델" not in body_text:
            pytest.skip("등록된 모델 없음 — 모델 등록 후 재실행")
        has_ranking = (
            "리더보드" in body_text or
            "순위" in body_text or
            "RANK" in body_text.upper() or
            "모델" in body_text
        )
        assert has_ranking, "리더보드 순위 컨텐츠가 없습니다"

    def test_live_benchmark_button_navigates_to_leaderboard(self, page: Page):
        """주문 입력 페이지의 'Live Benchmark' 버튼 클릭 시 리더보드로 이동한다."""
        page.goto(BENCHMARK_URL, wait_until="load")
        page.wait_for_timeout(500)
        page.get_by_role("button", name="Live Benchmark").click()
        page.wait_for_url("**/benchmark/models**", timeout=10000)
